...
public class EventController {
    ...
    public ResponseEntity<Object> patchTicketRegistrationById(
            @PathVariable RegistersId id,
            @Valid @RequestBody RegistersStatus status,
            BindingResult bindingResult,
            Authentication authentication) throws RegistrationNotFoundException 
    {
        if (bindingResult.hasErrors()) {
            return ResponseEntity.badRequest().body(new ResponseMessage(
                    "Validation failed: " + bindingResult.getAllErrors(), ResponseMessage.Status.ERROR));
        ...
        return ResponseEntity.ok(new ResponseMessage(eventService.patchTicketRegistrationById(id, status), ResponseMessage.Status.SUCCESS));
    }
